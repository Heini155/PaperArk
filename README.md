...
**Base32-Seiten:** Der Datenblock ist **links bündig** mit der Anleitung gesetzt; der linke Rand von 25 mm bleibt frei für Lochung. Schriftgrad 7 pt (wie die Anleitung). Seite 1 trägt die vollständige Wiederherstellungsanleitung und 62 Datenzeilen; die Folgeseiten haben einen kompakten Kopf und tragen bis zu 76 Zeilen. Die Zeilen auf den Folgeseiten werden gleichmäßig verteilt, damit keine halbleere Seite entsteht. Jede Zeile: 96 Datenzeichen + 8 CRC-Zeichen und füllt damit die volle Breite aus.
...
2. **Symbol-Interleaving:** Die Codewörter werden zeichenweise über die Seite verschachtelt. Eine vollständig unlesbare Zeile (60 Bytes bei 96 Zeichen) verteilt sich auf viele Codewörter und überlastet kein einzelnes. Die Anzahl komplett verkraftbarer Zeilen wächst mit der Archivgröße: `⌊40 · D / 60⌋` (D = Codewortanzahl).
...
**Stream-Aufbau:** 4 Byte Big-Endian-Länge L, dann L Bytes PaperArk-Blöcke, dann Null-Padding. Die Codewortanzahl ist ein Vielfaches von 4, dadurch ist die Zeichenzahl immer ein exaktes Vielfaches von 96 — jede gedruckte Zeile ist voll gefüllt, es existiert kein `=`-Padding, das bei Beschädigung der letzten Zeile die Stromlänge verschieben könnte.
